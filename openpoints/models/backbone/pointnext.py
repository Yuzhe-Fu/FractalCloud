...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        if not is_head:
            ...
            self.grouper = create_grouper(self.stage, group_args)
    ...
    def forward(self, pf):
        ...
        if self.is_head:
            ...
            if not self.all_aggr:
                ...
                # time0 = time.time()
                idx = self.sample_fn(p, p.shape[1] // self.stride, self.stage).long()
                # torch.cuda.synchronize()
                # time1 = time.time()
                # print('fps time: ', time1 - time0)
                # fps_time = time1 - time0
            ...
            #todo: need to add stage here
            # pdb.set_trace()
            #FIXME Ye: group right here
            # torch.cuda.synchronize()
            # time0 = time.time()
            dp, fj = self.grouper(new_p, p, f)
            # torch.cuda.synchronize()
            # time1 = time.time()
            # print('group time: ', time1 - time0)
            ...
            p = new_p
        return p, f
        # return p, f, fps_time # only for segmentation, ont for clf
...
class FeaturePropogation(nn.Module):
    ...
    def forward(self, pf1, pf2=None):
        ...
        if pf2 is None:
            ...
            p2, f2 = pf2
            if f1 is not None:
                #FIXME Interpolation here
                f = self.convs(
                    torch.cat((f1, three_interpolation(p1, p2, f2, self.stage)), dim=1))
            else:
                f = self.convs(three_interpolation(p1, p2, f2, self.stage))
        return f
...
class PointNextEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(blocks)):
            ...
            encoder.append(self._make_enc(
                stage = stage[i], block = block, channels=channels[i], blocks=blocks[i], stride=strides[i], group_args=group_args,
    ...
    def _make_enc(self, stage, block, channels, blocks, stride, group_args, is_head=False):
        layers = []
        radii = group_args.radius
        nsample = group_args.nsample
        group_args.radius = radii[0]
        group_args.nsample = nsample[0]
        layers.append(SetAbstraction(stage, self.in_channels, channels,
    ...
    def forward_seg_feat(self, p0, f0=None):
        ...
        for i in range(0, len(self.encoder)):
            ...
            p.append(_p)
            f.append(_f)
            # FPS_total_time += _fps_time
        # logging.info(f'FPS time in ms: {FPS_total_time * 1000} ms')
        return p, f
    ...
    def __init__(self,
        ...
        for i in range(-1, -n_decoder_stages - 1, -1):
            decoder[i] = self._make_dec(
                i, skip_channels[i], fp_channels[i])
        self.decoder = nn.Sequential(*decoder)
        self.out_channels = fp_channels[-n_decoder_stages]

    def _make_dec(self, stage, skip_channels, fp_channels):
        layers = []
        mlp = [skip_channels + self.in_channels] + \
              [fp_channels] * self.decoder_layers
        layers.append(FeaturePropogation(stage, mlp))
...
class PointNextPartDecoder(nn.Module):
    def __init__(self,
        ...
        for i in range(-1, -n_decoder_stages - 1, -1):
            ...
            group_args.nsample = self.nsample[i]
            decoder[i] = self._make_dec(
                i, skip_channels[i], fp_channels[i], group_args=group_args, block=block, blocks=self.blocks[i])

        self.decoder = nn.Sequential(*decoder)
        self.out_channels = fp_channels[-n_decoder_stages]

    def _make_dec(self, stage, skip_channels, fp_channels, group_args=None, block=None, blocks=1):
        layers = []
        radii = group_args.radius
        nsample = group_args.nsample
        mlp = [skip_channels + self.in_channels] + \
              [fp_channels] * self.decoder_layers
        layers.append(FeaturePropogation(stage, mlp, act_args=self.act_args))
        self.in_channels = fp_channels
        for i in range(1, blocks):
            group_args.radius = radii[i]
            group_args.nsample = nsample[i]
