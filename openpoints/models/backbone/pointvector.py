...
class LocalAggregation(nn.Module):
    ...
    def __init__(self,
        ...
        # pdb.set_trace()
        self.grouper = create_grouper(10, group_args)
    ...
    def __init__(self,
        ...
        if not is_head:
            if self.all_aggr:
                ...
                group_args.radius = None
                self.pool = lambda x: torch.max(x, dim=-1, keepdim=False)[0]
            self.grouper = create_grouper(stage, group_args)
...
class VPSA(nn.Module):
    ...
    def forward(self,position,identity,y):# y b,d,g,1
        ...
        # global POS_X
        # logging.info(f"POS_X: {POS_X}, input shape: {position.shape}")
        # POS_X +=1  
        ...
        y=self.relu(y)
        del position
        # global THETA_X_ALPHA
        # logging.info(f"THETA_X_ALPHA: {THETA_X_ALPHA}, input shape: {y.shape}")
        # THETA_X_ALPHA +=1
        theta_x_alpha=self.theta_x_alpha(y)

        # global THETA_X_BETA
        # logging.info(f"THETA_X_BETA: {THETA_X_BETA}, input shape: {y.shape}")
        # THETA_X_BETA +=1
        theta_x_beta=self.theta_x_beta(y)

        # global Z_X
        # logging.info(f"Z_X: {Z_X}, input shape: {y.shape}")
        # Z_X +=1
        zx=self.z_x(y)
        del y
        ...
        # global TF_ZX
        # logging.info(f"Z_X: {TF_ZX}, input shape: {zx.shape}")
        # TF_ZX +=1
        zx=self.tf_zx(zx)#b,d,g,1

        # global RESULTS
        # logging.info(f"RESULTS: {RESULTS}, input shape: {zx.shape}")
        # RESULTS +=1
        zx=self.result(zx)

        # global RESIDUAL
        # logging.info(f"RESIDUAL: {RESIDUAL}, input shape: {residual.shape}")
        # RESIDUAL +=1       
...
class PointVectorEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(blocks)):
            ...
            encoder.append(self._make_enc(
                i, block, channels[i], blocks[i], stride=strides[i], group_args=group_args,
    ...
    def _make_enc(self, stage, block, channels, blocks, stride, group_args, is_head=False):
        ...
        if self.flag == 1:
            ...
            layers.append(SetAbstractioncls(stage, self.in_channels, channels,
                                     self.sa_layers if not is_head else 1, stride,
                                     group_args=group_args,
                                     sampler=self.sampler,
                                     norm_args=self.norm_args, act_args=self.act_args, conv_args=self.conv_args,
