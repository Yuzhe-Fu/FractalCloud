...
class PointNetSAModuleMSG(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(radii)):
            ...
            self.local_aggregations.append(
                LocalAggregation(self.stage, channels, aggr_args, conv_args, norm_args, act_args,
                                 group_args, use_res))
    ...
    def forward(self,
        ...
        if query_xyz is None and self.stride > 1:
            idx = self.sample_fn(
                support_xyz, support_xyz.shape[1] // self.stride, stage).long()
...
class PointNetFPModule(nn.Module):
    ...
    def __init__(self, stage, mlp: List[int],
                 norm_args={'norm': 'bn1d'},
                 act_args={'act': 'relu'},
                 ):
        """
    ...
    def forward(
        ...
        if known is not None:
            interpolated_feats = three_interpolation(
                unknown, known, known_feats, self.stage) #FIXME
        else:
...
class PointNet2Encoder(nn.Module):
    ...
    def forward_cls_feat(self, xyz, features=None):
        ...
        for i in range(len(self.SA_modules)):
            ...
            xyz, features = self.SA_modules[i](i, xyz, features)
    ...
    def forward_seg_feat(self, xyz, features=None):
        ...
        for i in range(len(self.SA_modules)):
            # query_xyz is None. query for neighbors
            li_xyz, li_features = self.SA_modules[i](i, l_xyz[i], l_features[i])
            l_xyz.append(li_xyz)
    ...
    def __init__(self,
        ...
        for k in range(fp_mlps.__len__()):
            ...
            self.FP_modules.append(
                PointNetFPModule(
                    k, [pre_channel + skip_channel_list[k]] + fp_mlps[k]
                )
            )
