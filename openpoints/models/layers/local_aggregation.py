...
class ConvPool(nn.Module):
    ...
    def forward(self, query_xyz, support_xyz, features, query_idx=None):
        ...
        fj = self.convs(fj)
        # logging.info(f'MAXPOOL-{MAXPOOL_COUNT}: {fj.shape}')
        # MAXPOOL_COUNT += 1
        out_features = self.reduction_layer(fj)
...
class LocalAggregation(nn.Module):
    def __init__(self,
        ...
        if aggr_type.lower() == 'convpool':
            self.SA_CONFIG_operator = ConvPool(self.stage, channels, conv_args, norm_args, act_args,
                                               group_args, feature_type, reduction, use_res, use_pooled_as_identity)
        elif aggr_type.lower() == 'assa':
            self.SA_CONFIG_operator = ASSA(self.stage, channels, conv_args, norm_args, act_args,
