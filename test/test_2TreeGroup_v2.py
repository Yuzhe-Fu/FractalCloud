...
def part2_and_count_v2(xyz, FPS_th, TreeDepth):
...
def part2_and_count(xyz, FPS_th, TreeDepth):
    ...
    for i in range(2):
        indx = torch.nonzero(xyz_0_temp[0,:]==i).reshape([1,-1]).unsqueeze(-1).expand(-1, -1, 3).to(torch.int64)
        xyz_row = (torch.gather(torch.reshape(xyz[0,:], [1,-1,3]), 1, indx))
        xyz_2dList[i] = xyz_row
        size_2dTensor[i] = indx.size()[1]
...
def mapping_from_sub_to_whole(xyz_L1, xyz, indx_xyz_L1):
    mapping = {}
                ...
                if (size_2dL_L1[j] != 0):
                    ...
                    if (FPS_2dL_L1[j]==1):
                        ...
                        if(centerXYZ_size != 0):
                            ...
                            indx_block = mapping_from_sub_to_whole(group_from_xyz, xyz, idx)
                            indx_batch = torch.cat((indx_batch, indx_block), dim=1)
                        else:
                            continue
                    else: # deep = 2
...
# 迭代方式
xyz_out_ours =  Tree_group_depth10_config(xyz_0, 32, new_xyz, 0.15, 128) #(xyz, npoint, blockNum, scale, PMS=1, BMS=1)
