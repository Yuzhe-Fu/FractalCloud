...
# def process_branch(xyz_2dL_L1, size_2dL_L1, nsample, new_xyz, radius, FPS_2dL_L1, FPS_th, i, tree_depth, ori_index, executor=None):
#     if size_2dL_L1[i] != 0:
#         xyz_L1 = xyz_2dL_L1[i]
#         if FPS_2dL_L1[i] == 1:
#             centerXYZ, centerXYZ_size, indices = selectAfromB(xyz_L1, new_xyz[i].unsqueeze(0))
#             if(centerXYZ_size != 0):
#                 group_from_xyz = xyz_L1
#                 group_from_xyz_N = xyz_L1.size(1)
#                 idx = torch.cuda.IntTensor(1, centerXYZ_size, nsample, device=xyz_2dL_L1.device).zero_()
...
#                 indices_batch = torch.cat((indices_batch, indices), dim=0)
#                 global_index = torch.gather(ori_index, dim=1, index=idx)
#                 indx_batch = torch.cat((indx_batch, global_index), dim=1)
#         else:
#             indices_batch, indx_batch = TreeBlock_group_recursive_config(
#                 xyz_L1, nsample, new_xyz, radius, FPS_th, tree_depth, ori_index, executor
#             )
#         return indices_batch, indx_batch
#     else:
#         return None, None
    

def process_branch(xyz_2dL_L1, size_2dL_L1, nsample, new_xyz, radius, FPS_2dL_L1, FPS_th, i, j, tree_depth, ori_index, executor=None, center_idx_all_batch_list=[], group_idx_all_batch_list=[]):
    if size_2dL_L1[j] != 0:
        xyz_L1 = xyz_2dL_L1[j]
        if FPS_2dL_L1[j] == 1:
            centerXYZ, centerXYZ_size, indices_center = selectAfromB(xyz_L1, new_xyz[i].unsqueeze(0))
            if(centerXYZ_size != 0):
                ...
                # indx_batch = torch.cat((indx_batch, idx), dim=1)
                # indices_batch = torch.cat((indices_batch, indices), dim=0)
                # if idx.shape[-1] > ori_index.shape[-1]:
                #     ori_index = torch.cat([ori_index, ori_index[..., 0:1].repeat(1, idx.shape[-1] - ori_index.shape[-1])], dim=-1)
                # ori_index = ori_index.unsqueeze(1).expand_as(idx)
                ...
                # print(f'Batch {i}, Node {j}: ori_index:{ori_index.cpu().data.numpy()}\n idx_group: {idx.cpu().data.numpy()}')
                    
                # pdb.set_trace()
                # ori_index = ori_index.unsqueeze(1).expand_as(idx)
                global_index_group =  torch.gather(ori_index.unsqueeze(1).unsqueeze(1).expand(-1, idx.size(1), idx.size(2), -1), dim=-1, index=idx.long().unsqueeze(-1)).squeeze(-1)
                # indx_batch = torch.cat((indx_batch, global_index), dim=1)
...
def TreeBlock_group_recursive_config(xyz, nsample, new_xyz, radius, FPS_th, tree_depth=0, global_index=None, executor=None, index_global_group=None, index_global_center=None, center_idx_all_batch_list=[], group_idx_all_batch_list=[]):
    ...
    for i in range(B):  # Loop over batches
        ...
        futures = [executor.submit(process_branch, xyz_2dL_L1, size_2dL_L1, nsample, new_xyz, radius, FPS_2dL_L1, FPS_th, i, j, tree_depth + 1, split_index[j], executor, [], []) for j in range(2)]
    ...
    if len(center_idx_all_batch_list) == new_xyz.shape[0]:
        # pdb.set_trace()
        return indices_center_batch, indx_group_batch, torch.stack(center_idx_all_batch_list), torch.cat(group_idx_all_batch_list, dim=0)
    else:
...
def Tree_group_depth10_config(xyz, nsample, new_xyz, radius):
    ...
    try:
        for i in range(0, B): # deep=1
                    ...
                    if (FPS_2dL_L1[j]==1):
                        ...
                        for k in range(0, 2):
                            if (size_2dL_L2[k] != 0):
                                xyz_L2 = xyz_2dL_L2[k]
                                if (FPS_2dL_L2[k]==1):
                                    centerXYZ, centerXYZ_size, indices = selectAfromB(xyz_L2, new_xyz[i,:,:].unsqueeze(0))
                                    if(centerXYZ_size != 0):
                                    ...
                                    for l in range(0, 2):
                                        if (size_2dL_L3[l] != 0):
                                            xyz_L3 = xyz_2dL_L3[l]
                                            if (FPS_2dL_L3[l]==1):
                                                centerXYZ, centerXYZ_size, indices = selectAfromB(xyz_L3, new_xyz[i,:,:].unsqueeze(0))
                                                if(centerXYZ_size != 0):
                                                ...
                                                for m in range(0, 2):
                                                    ...
                                                    if (size_2dL_L4[m]!=0):
                                                        xyz_L4 = xyz_2dL_L4[m]
                                                        if (FPS_2dL_L4[m]==1):
                                                            centerXYZ, centerXYZ_size, indices  = selectAfromB(xyz_L4, new_xyz[i,:,:].unsqueeze(0)) # group source
                                                            if(centerXYZ_size != 0):
                                                            ...
                                                            for n in range(0, 2):
                                                                if (size_2dL_L5[n]!=0):
                                                                    xyz_L5 = xyz_2dL_L5[n]
                                                                    if (FPS_2dL_L5[n]==1):
                                                                        centerXYZ, centerXYZ_size, indices  = selectAfromB(xyz_L5, new_xyz[i,:,:].unsqueeze(0)) # group source
                                                                        if(centerXYZ_size != 0):
                                                                        ...
                                                                        for o in range(0, 2):
                                                                            if (size_2dL_L6[o] != 0):
                                                                                xyz_L6 = xyz_2dL_L6[o]
                                                                                if (FPS_2dL_L6[o]==1):
                                                                                    centerXYZ, centerXYZ_size, indices = selectAfromB(xyz_L6, new_xyz[i,:,:].unsqueeze(0)) # group source
                                                                                    if(centerXYZ_size != 0):
                                                                                    ...
                                                                                    for p in range(0, 2):
                                                                                        if (size_2dL_L7[p] != 0):
                                                                                            xyz_L7 = xyz_2dL_L7[p]
                                                                                            if (FPS_2dL_L7[p]==1):
                                                                                                centerXYZ, centerXYZ_size, indices = selectAfromB(xyz_L7, new_xyz[i,:,:].unsqueeze(0)) # group source
                                                                                                if(centerXYZ_size != 0):
                                                                                                ...
                                                                                                for q in range(0, 2):
                                                                                                    if (size_2dL_L8[q] != 0):
                                                                                                        xyz_L8 = xyz_2dL_L8[q]
                                                                                                        if (FPS_2dL_L8[q]==1):
                                                                                                            centerXYZ, centerXYZ_size, indices = selectAfromB(xyz_L8, new_xyz[i,:,:].unsqueeze(0)) # group source
                                                                                                            if(centerXYZ_size != 0):
                                                                                                            ...
                                                                                                            for r in range(0, 2):
                                                                                                                if (size_2dL_L9[r] != 0):
                                                                                                                    xyz_L9 = xyz_2dL_L9[r]
                                                                                                                    if (FPS_2dL_L9[r]==1):
                                                                                                                        centerXYZ, centerXYZ_size, indices = selectAfromB(xyz_L9, new_xyz[i,:,:].unsqueeze(0)) # group source
                                                                                                                        if(centerXYZ_size != 0):
                                                                                                                        ...
                                                                                                                        for s in range(0, 2):
                                                                                                                            if (size_2dL_L10[s] != 0):
                                                                                                                                ...
                                                                                                                                centerXYZ, centerXYZ_size, indices = selectAfromB(xyz_L10, new_xyz[i,:,:].unsqueeze(0)) # group source
                                                                                                                                if(centerXYZ_size != 0):
            ...
            # if(i==23):
            #     # pdb.set_trace()
            sorted_indices = torch.argsort(indices_batch)
            indx_batch_reordered = indx_batch[:, sorted_indices, :]
            # pdb.set_trace()

            indx_all = torch.cat((indx_all, indx_batch_reordered), dim=0)
...
_, _, indices_global, indx_global =  TreeBlock_group_recursive_config(xyz_0, 32, new_xyz, 0.15, FPS_th) #(xyz, npoint, blockNum, scale, PMS=1, BMS=1)
# pdb.set_trace()
...
bn_recursive = blockNum_count(xyz_0, xyz_out_recursive, BlockNum_MAE)
bn_ori = blockNum_count(xyz_0, xyz_out_original, BlockNum_MAE)

sum_16b_total = 0
sum_our_total = 0
sum_recursive_total = 0

md_16b_total = 0
md_our_total = 0
md_recursive_total = 0
md_new_total = 0

xyz_out_original = xyz_out_original.to(torch.int64)
# xyz_out_16block = xyz_out_16block.to(torch.int64)
xyz_out_ours = xyz_out_ours.to(torch.int64)
xyz_out_recursive = xyz_out_recursive.to(torch.int64)
...
xyz_out_ours_point = torch.gather(xyz_0, 1, xyz_out_recursive.unsqueeze(-1).expand(-1, -1, 3)) #(64,256,3)
...
MAE = sum_recursive_total/(BlockNum_MAE*64)
IMD = md_recursive_total/64
Acc = MAE+IMD
